...
const Modal = ({order, arrowVisible, print}) => {
  const [dropdown, setDropdown] = useState(false);

  const printOrderReceipt= ()=>{
    let mywindow= receipt(order, Logo1)
    mywindow.print();
    mywindow.close();
  }
    ...
    <div> 
      ...
      <div className={`mt-5 pb-5 rounded-[30px] z-30 shadow-[1px_-2px_51px_-12px_rgba(0,0,0,0.25)]`}>
        ...
        >
          ...
          {print ? (
            <div>
              <div className="border-b-[1px] border-[black] border-dotted mt-8"></div>
              <div className="flex items-center justify-between mt-2">
                <h1 className="text-[#4964D8] font-bold">Print receipt</h1>
                <div onClick={printOrderReceipt} className="shadow-[0px_1px_4px_rgba(0,0,0,0.25)] p-2 rounded-full cursor-pointer">
